...
function Mapa(){
  ...
  this.leeJSON = function(){
    // $.getJSON("public/js/json/catalogo.json", function(data){}
      $.getJSON("public/js/json/catalogomiguel2.json", function(data){
      var html = [];
      ...
      for (i = 0; i < html.length; i++){
        ...
        if (html[i]['NIVEL']=='ESPECIAL') {image = 'public/img/marker1.png'};
        if (html[i]['NIVEL']=='INICIAL') {image = 'public/img/marker2.png'};
        if (html[i]['NIVEL']=='PREESCOLAR') {image = 'public/img/marker3.png'};
        if (html[i]['NIVEL']=='PRIMARIA') {image = 'public/img/marker4.png'};
        if (html[i]['NIVEL']=='SECUNDARIA') {image = 'public/img/marker5.png'};
        if (html[i]['NIVEL']=='MEDIA SUPERIOR') {image = 'public/img/marker6.png'};
        if (html[i]['NIVEL']=='SUPERIOR') {image = 'public/img/marker7.png'};
        if (html[i]['NIVEL']=='FORMACION PARA TRABAJO') {image = 'public/img/marker8.png'};
        ...
        for(j = 0; j < alta_demanda.length; j++){
          ...
          contentString = '<center>NOMBRE ESCUELA: <b style="font-size: 150%;">'+html[i]['NOMBRECT']+
          '</b><br>CCT: <b>'+html[i]['CLAVECCT']+
          ',</b> NIVEL: <b>'+html[i]['NIVEL']+
          ',</b> SOSTENIMIENTO: <b>'+html[i]['SOSTENIMIE']+
          '<br></b>LOCALIDAD: <b>'+html[i]['NOMBRELOC']+
          ', </b>MUNICIPIO: <b>'+html[i]['NOMBREMUN']+
          '<br></b> TURNO: <b>'+html[i]['TURNO']+
          '</b>, ZONA: <b>'+html[i]['ZONA']+
          '<br> <b style="font-size: 120%; color:red;">'+estatusesc+'<b>'+
          '</b><b><input id="mostar_cercanas"  onclick="funcionckct('+i+')"  type="button"value="Localice 5 escuelas más cercanas del mismo nivel educativo" /></b>'+
          '<b><input id="mostar_cercanas_signv"  onclick="funcionckct_signv('+i+')" title="Ejem: 5 escuelas secundarias más cercanas a la primaria donde estudia mi hija(o)"  type="button"  value="Localice 5 escuelas más cercanas del siguiente nivel educativo" /></b>'+
          ...
          '<b><input  type="button" onclick="funcionjsonct('+i+')" id="estadistica" value="Conozca información relevante de esta escuela" ></b>'+
          '</form></center>';
...
function funcionckct_signv(obj) {
  ...
  for (i = 0; i < marker.length; i++) {
    ...
    if(calcDistance(marker[obj].getPosition(),marker[i].getPosition()) !=0 && temp_icon==marker[i].getIcon()){
      distancias[i] = calcDistance(marker[obj].getPosition(),marker[i].getPosition());
    }
  }
  ...
  for (i = 0; i < marker.length; i++) {
    if (temp_icon==marker[i].getIcon()) {
      if (calcDistance(marker[obj].getPosition(),marker[i].getPosition())==distancias[0]  ||
      calcDistance(marker[obj].getPosition(),marker[i].getPosition())==distancias[1]  ||
      calcDistance(marker[obj].getPosition(),marker[i].getPosition())==distancias[2]  ||
      calcDistance(marker[obj].getPosition(),marker[i].getPosition())==distancias[3]  ||
      calcDistance(marker[obj].getPosition(),marker[i].getPosition())==distancias[4]) {
        marker[i].setMap(map);
      }
      else{
        marker[i].setMap(null);
...
function funcionckct(obj) {
  ...
  for (i = 0; i < marker.length; i++) {
    ...
    if(calcDistance(marker[obj].getPosition(),marker[i].getPosition()) !=0 && marker[obj].getIcon()==marker[i].getIcon()){
      distancias[i] = calcDistance(marker[obj].getPosition(),marker[i].getPosition());
    }
