...
$("#LM_btn_rebuscar").click(function(e){
  e.preventDefault();
  // El valor de los select a la primer posición
  $("#LM_select_idmunicipio").val(0);
  $("#LM_select_nivel").val(0);
  $("#LM_select_sostenimiento").val(0);
  $("#LM_select_fpa").val(0);
  // Y muestro el modal
  obj_LE.clearMap();
  $("#LM_modal").modal("show");
...
$("#LM_modal_btnayuda").click(function(e){
  e.preventDefault();
  $("#LM_ayuda").modal("show");
});
...
$("#le_modal_infoescuela_btn_salir").click(function(e){
  e.preventDefault();
    ...
    $("#le_modal_infoescuela").modal("hide");
});
  ...
  console.table(marker);
    ...
    this.validaBusqueda = function(){
      window.scrollTo(0,200);
        ...
        var id_programa_federal   = $("#LM_select_fpa").val();
        var nombre_programa_federal   = $("#LM_select_fpa option:selected").text();

        var escuela = (nombre_escuela.trim().length>0)?nombre_escuela:"No especificado";
        $("#LM_resultados_busqueda").text("Municipio: "+nombre_municipio+",  Nivel: "+nombre_nivel+",  Sostenimiento: "+sostenimiento+",  Programas Federales de Apoyo: "+nombre_programa_federal+",  Escuela: "+escuela);

        var cct =  $("#LM_text_CCT").val();
        obj_LE.localizaEscuelasMapa(id_programa_federal,id_municipio,nombre_municipio,id_nivel,nombre_nivel,sostenimiento,nombre_escuela,cct);
    }// validaBusqueda()

    this.localizaEscuelasMapa = function(id_programa_federal,id_municipio,nombre_municipio,id_nivel,nombre_nivel,sostenimiento,nombre_escuela,cct){
        ...
        obj_LE.posicionaEnNivel(nombre_nivel,sostenimiento, function(result){
                obj_LE.posicionaEnSostenimiento(result,sostenimiento, function(result2){
                    obj_LE.posicionaEnprog_fed(result2,id_programa_federal, function(result3){
                      obj_LE.posicionaEnNombreEscuela(result3,nombre_escuela,function(result4){
                          obj_LE.posicionaFinal(result4);
                      });
                    });
                });
        });
    ...
    this.posicionaEnNivel = function(nivel, sostenimiento,callback){
            ...
            // if (marker[i].getIcon()==ruta_markets+'marker1.png') { markers_aux.push(  marker[i]); }
            ...
            if(nivel=="ESPECIAL"){
              ...
              for (var i = 0; i < marker.length; i++) {
                  ...
                  if (marker[i].getIcon()==ruta_markets+'marker2.png') { markers_aux.push(  marker[i]); }
              }
          }
          else if(nivel=="PREESCOLAR"){
                    for (var i = 0; i < marker.length; i++) {
                        // if (marker[i].getIcon()=='public/img/marker3.png') { markers_aux.push(  marker[i]); }
                        if (marker[i].getIcon()==ruta_markets+'marker3.png') { markers_aux.push(  marker[i]); }
                    }
          }
                    ...
                    for (var i = 0; i < marker.length; i++) {
                        ...
                        if (marker[i].getIcon()==ruta_markets+'marker4.png') { markers_aux.push(  marker[i]); }
                    }
          }
                  ...
                  for (var i = 0; i < marker.length; i++) {
                    ...
                    for (var i = 0; i < marker.length; i++) {
                        ...
                        if (marker[i].getIcon()==ruta_markets+'marker8.png') { markers_aux.push(  marker[i]); }
                    }
          }
    ...
    this.posicionaEnSostenimiento = function(arr_nivel, sostenimiento,callback){
            ...
            if(sostenimiento=="AUTONOMO"){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if ("AUTONOMO" == arr_nivel[i]['sostenimiento']) { markers_aux2.push(arr_nivel[i]); }
                }
            }
    ...
    this.posicionaEnprog_fed = function(arr_nivel, id_programa_federal,callback){
            console.info("posicionaEnprog_fed()->id_programa_federal: "+id_programa_federal);

            var markers_aux2 = Array();

            if(id_programa_federal==0){
                    for (var i = 0; i < arr_nivel.length; i++) {
                          markers_aux2.push(arr_nivel[i]);
                    }
            }

            if(id_programa_federal==1){
                    for (var i = 0; i < arr_nivel.length; i++) {
                        if ((arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('2')!=-1) ||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('3')!=-1)||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('4')!=-1)||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('5')!=-1)||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('6')!=-1)||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('7')!=-1)||
                         (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('8')!=-1)) { markers_aux2.push(arr_nivel[i]); }
                    }
            }

            if(id_programa_federal==2){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('2')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==3){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('3')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==4){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('4')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==5){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('5')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==6){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('6')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==7){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('7')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
            if(id_programa_federal==8){
                for (var i = 0; i < arr_nivel.length; i++) {
                    if (arr_nivel[i]['cad_prog_apoyo_fer'].indexOf('8')!=-1) { markers_aux2.push(arr_nivel[i]); }
                }
            }
...
function getInfoEscuela(cct, turno){
  ...
  $.ajax({
    url: ruta,
    method: 'POST',
    data: {'cct':cct,'turno':turno,'action':'busqueda_xmapa'},
    beforeSend: function( xhr ) {
      obj_message.loading("Descargando datos");
    }
  })
...
function hace_graficas(arr_graficas, nivel_g, callback){
  ...
  case "SECUNDARIA":
          ...
          if (arr_graficas['graph_unidad_analisis_lyc']!='') {
          graf.graficoplanea_ud_secu_lyc(arr_graficas['graph_unidad_analisis_lyc']); // Por Unidades de Análisis lyc
          graf.graficoplanea_ud_secu_mate(arr_graficas['graph_unidad_analisis_mate']); // Por Unidades de Análisis mate
          }
          aux=1;
...
function infoEscuela(){
  ...
  $("#top_modal").click(function(e){
    e.preventDefault();
    $('#le_modal_infoescuela').scrollTop(0);
  });

  $('#le_modal_infoescuela').scroll(function(){
      if ($('#le_modal_infoescuela').scrollTop() > 400) {
